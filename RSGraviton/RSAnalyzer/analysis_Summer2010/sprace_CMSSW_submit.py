...
if len(sys.argv) < 4:  # the program name and the config file
	...
	sys.exit("Usage: sprace_CMSSW_submit.py <config file> <seq number> <eventblocksize> <many parameters>")
...
cmsswEventBlockSize = str(sys.argv[4])
cmsswEventsToSkip = str(int(sys.argv[2])*int(sys.argv[4]))
print cmsswEventsToSkip
taskname = label
...
shellfile.write('cmsRun '+configname+
#		' ptCut='+str(sys.argv[3])+
#		' etaCut='+str(sys.argv[4])+
#		' massCut='+str(sys.argv[5])+
#		' metCut='+str(sys.argv[6])+
		' fileLabel='+str(sys.argv[3])+
		' skipEvents='+cmsswEventsToSkip+
		' maxEvents='+cmsswEventBlockSize+
		' print\n')
