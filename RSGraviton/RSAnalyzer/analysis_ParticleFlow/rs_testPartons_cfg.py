...
process.hardestParton = cms.EDFilter("LargestPtCandViewSelector",
                                     src = cms.InputTag("partons"),
                                     maxNumber = cms.uint32(1)
                                     )

process.plotPartonsBefore = cms.EDAnalyzer("CandViewHistoAnalyzer",
                                           src = cms.InputTag("hardestParton"),
                                           histograms = cms.VPSet(cms.PSet(
                                               nbins = cms.untracked.int32(100),
                                               description = cms.untracked.string('jet_pt'),
...
# Make the jets.
process.p1 = cms.Path(process.partons + process.hardestParton +
                      process.plotPartonsBefore +
                      process.doSisConeJets +
                      process.cutsT1 +
                      process.cutsT2 +
                      process.doGravitons +
                      process.cutsT3 +
                      process.plotPartonsAfter
                      )
