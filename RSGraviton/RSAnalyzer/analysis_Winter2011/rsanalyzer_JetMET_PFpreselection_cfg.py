...
myFilesList = filelist1[startFile-1:endFile:]
...
process.jetIdCut = cms.EDFilter("RSPFJetIdSelector",
                                ...
                                correctorName = cms.string("ak5PFL2L3Residual"),
...
process.myL2L3ResCorJetAK5PF = cms.EDProducer('PFJetCorrectionProducer',
                                              src        = cms.InputTag('jetIdCut'),
                                              correctors = cms.vstring('ak5PFL2L3Residual')
                                              )
process.myCorrections = cms.Sequence(process.myL2L3ResCorJetAK5PF)
...
process.oneJetAboveZero = cms.EDFilter("PFJetConfigurableSelector",
                                       src = cms.InputTag("myL2L3ResCorJetAK5PF"),
...
process.thiagoSelection = cms.Sequence(
    process.eventCounter + process.goodVertexSequence + process.noiseFilters + # Good vertex and no noise
    process.eventCounterTwo + process.jetId +
    process.eventCounterThree + process.cuts0 + process.getLargestJet +
    process.eventCounterFour + process.cuts1 + process.cuts2 # Jet must pass pt and eta  cut
    )
